...
- **"角标"（`⌈39⌋` 那种左上 + 右下、右上/左下敞口）要用画的，别用字形（2026-09-22）**：`U+2308` / `U+230B`（LEFT CEILING / RIGHT FLOOR）在 Segoe UI 里没有，WinUI 走**字体回退**取字形，而回退字形的 `FontWeight=Bold` 请求**不生效** → 笔画比加粗标题细一档（实拍对比后否掉）。做法：**两个只画两条边的 10×10 `Border`**，分别贴数字框的 `HorizontalAlignment=Left/VerticalAlignment=Top`（`BorderThickness="2,2,0,0"`）与 `Right/Bottom`（`"0,0,2,2"`），数字框自己用 `Margin` 留白 —— 粗细/臂长/留白全可控、不依赖字体，网格尺寸自动跟着文字走（`LibraryPage` 的入库数量角标）
