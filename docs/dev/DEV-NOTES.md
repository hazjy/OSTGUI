...
> 其余按分工落位：**机制语义就近写代码注释（首选）**；领域机制 / 自有实现事实 / 实测结论 → 工作区 `doc/GUI-事实考证.md`；UI 尺寸·文案·图标·交互偏好 → `doc/细节与偏好.md`；环境 / 工具链 / 打包 → `doc/开发踩坑-环境.md`，WinUI 框架坑与验证手法 → `doc/开发踩坑-UI.md`；内核侧机制 → `ZSteamTool/docs/dev/DEV-NOTES.md`。工程状态快照（构建产物、临时数字、"待提交"）不收。
...
- **搜索 → 入库链路**：搜索（Steam 官方 API 主源）→ 取 depot / manifest gid / DLC（`SteamGameInfoService`）→ 清单下载并**双写** `config\depotcache` 与 Steam 根 `depotcache`（`ManifestDownloadService` + `ManifestFileService`）→ 密钥 / 令牌（`SudamaKeyCache`）→ 生成 Lua（`LuaBuilder` → `LuaConfigService`）；**入库可取消**（`ct` 贯穿全链，两处原子写不可打断）。出处：`doc/GUI-事实考证.md` 的「搜索与入库链路」「Sudama 密钥缓存」「清单投喂位置」
- **固定版本体系**：GUI 只写 Lua（注释形式 `--setManifestid(...)` = 固定版本配置），实际锁版本由内核 hook 完成；库页切锁定模式（`LuaConfigService.ToggleVersionModeAsync`，要求 depot 全覆盖）与「补齐版本配置」（`RepairVersionConfigAsync`）。出处：`doc/GUI-事实考证.md`「固定版本体系对应实现」+ 内核 DEV-NOTES
- **免 Steam 部署（NoSteamLauncher）**：Steamless 脱壳 + GSE(Goldberg) 部署 + 可选 Bypass，另有「一键还原」；三层 = 宿主类库 / GBE 部署服务 / 编排器。出处：`doc/GUI-事实考证.md`「免 Steam 部署」
- **联机（三条路线）**：`OnlineFixService` 统一入口，**一次只走一条** —— ① 内核原生（`steam.exe -applaunch <游戏> -onlinefix=<会话 AppID>`，靠读 PEB 扫命令行检测 / 停止）② 宿主 480（拉起 `OnlineHost.exe`，游戏 exe 按 AppID 自动解析）③ AppID Changer（`--appid-txt` 写 `steam_appid.txt` + 台账还原）。②③ 共用 `IsHostRunning` 判并发；内核侧只有一份会话状态。出处：`doc/GUI-事实考证.md` 的「480 联机邀请失效（v4）」「AppID Changer」「联机路线 A」
- **封面图**：`CoverImageService` 只返回文件路径（不碰 WinUI 类型），位图由 VM 在 UI 线程构造；入库封面落盘 `%LOCALAPPDATA%\OSTGUI\covers\`（缺失标记 `.miss2`），**搜索页缩略图刻意只走内存不落盘**；列表 / 网格共用同一份位图，切档只切宿主可见性。出处：`doc/GUI-事实考证.md`「封面图」；尺寸与放置规则见 `doc/细节与偏好.md`
...
- **成就编辑（成就页）**：左侧 = `LibraryScanner` 扫出的入库游戏；成就定义读本地 `<Steam>\appcache\stats\UserGameStatsSchema_<appid>.bin`（二进制 KV，`SteamStatsSchema`）。勾选**只写本地留底** `%LOCALAPPDATA%\OSTGUI\achievements\<appid>.json`；点「保存到 Steam」才 spawn `OSTGUI.exe --stats-apply`（`SteamStatsChild`，短命子进程 + 结果 JSON 文件，理由与 `SteamTicketExtractor` 相同）用 SAM 封装（`main/SteamApi/`，zlib）→ `ISteamUserStats013` 写回。**写入会进 Valve（重启 Steam 后仍在）**，但内核会对 addappid 游戏清空 819 里的成就数据 → 成就页可能显示不出来（显示层问题，不是没写进去）；证据与边界见 `doc/GUI-事实考证.md`「成就链路」
...
- 已知实现缺陷清单（`addtoken` 键错配、两条 lua 写入路径不一致、部署备份覆盖等）见 `doc/GUI-事实考证.md` 末节

## 5. 文档索引

- 领域机制 / 自有实现事实 / 踩坑定论（GUI 侧）：工作区 `doc/GUI-事实考证.md`
- 重要事件与调研：工作区 `doc/EVENTS/`
- UI 尺寸 / 文案 / 图标 / 交互偏好：工作区 `doc/细节与偏好.md`
- 环境 · 工具链 · 打包：工作区 `doc/开发踩坑-环境.md`；WinUI 框架坑与验证手法：`doc/开发踩坑-UI.md`
- 按日台账：`docs/dev/agents-log/`（本地留存，不纳入 git）；版本更新说明：`docs/changelog/`
- 内核侧：`ZSteamTool/docs/dev/DEV-NOTES.md` 与内核侧事实考证
