...
- 线程与解码宽度：`CoverImageService` 只返回文件路径（不碰 WinUI 类型）；`BitmapImage` 由 VM 在 UI 线程构造，`DecodePixelWidth` **恒为 200**（= 网格图框的逻辑宽）+ `DecodePixelType=Logical`（不设就是按 460×215 全量解码，几十张几十 MB）。⚠️ **不要"按当前视图换解码宽度"**：位图两档共用、只建一次，切档只是换宿主显示它；按视图来回重建的话，任何一次没重建到（`ContainerContentChanging` 只在容器重新分配时触发，切档时未必触发），卡片就会带着另一档的宽度显示 —— 网格里 1.67× 放大 = 糊，2026-09-22 的"时糊时清"就是这么来的。列表用这张 450 物理像素的位图只是降采样，比按 120 解更锐。⚠️ 这套只对**文件路径**（`UriSource`）成立；**流解码**路径（搜索页缩略图）设它会失效并发糊，见工作区 `doc/开发踩坑.md` 的 WinUI 小节
