...
> 收录原则：**项目结构与工程现状长期保留**；领域事实/机制查证/调研结论提取至 `../../doc/GUI-事实考证.md`（Lua 语义、depot/manifest/key 关系、Denuvo 授权、480 联机调研等）；工程状态快照（构建命令产物、临时数字、"待提交"类状态）不再收录，避免过时。
...
## 10. 创意工坊（Workshop）下载（Phase 1 已落地）
...
- 事实考证（Lua 语义 / depot·manifest·key 关系 / Denuvo 授权 / 480 联机调研）：`../../doc/GUI-事实考证.md`
