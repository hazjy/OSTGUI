...
- **入库取键走"流式扫描"（2026-09-22，内存优化，两轮才到位）**：缓存 23.9 万条 / 17.5MB。三种做法的实测对比（同一个文件、同一批 400 个 id、Release，量进程私有内存增量）：
  | 做法 | 入库期间内存增量 | 结果 |
  |---|---|---|
  | 整份物化成 `Dictionary<string,string>`（原实现） | ~110MB | — |
  | `JsonDocument`（DOM）惰性点查（第一版） | **+41.1MB**（DOM 必须整份驻留：字节 17.5MB + 元数据表） | 正确 |
  | **流式扫一遍只留命中项（现行）** | **+6.1MB** | 与 DOM 逐条一致（305/400 命中、0 不一致） |
  `SudamaKeyCache.ScanWantedAsync`：64KB 分块喂 `Utf8JsonReader`、跨块靠 `CurrentState` 续读，只收集 `wantedIds` 命中的键值对；调用方（`LuaBuilder`）**先算出要哪些 id**（appId + 各 depot + 各 DLC），所以 DLC 列表的获取被提到写行之前。
  ⚠️ **截断保护**：流式读不完整 JSON 时 `Read()` 可能只是返回 false（不抛），会把截断缓存静默当成"少了那些键"→ 入库静默少密钥。所以先做**尾部校验**（最后一个非空白字节必须是 `}`），不合格就抛 `JsonException` 让调用方降级（重下 / 旧缓存 / 空）。
- **缓存文件形状兼容两种**：本程序写的是 `{"Data":{…}}`，读侧也认原始明文 `{…}`（扫描只认"深度 ≤ 2 的键值对"，不关心套在哪一层）。
- **峰值后手动还内存**：`OstMemory.CompactAfterLargeBuffers()`（`LargeObjectHeapCompactionMode = CompactOnce` + 一次 `GC.Collect(2, Optimized, blocking, compacting)`，后台线程）在**入库结束**与**手动刷新缓存后**各调一次——GC 默认不移动大对象，不压的话峰值过去后空洞仍占着已提交内存（任务管理器上像泄漏）。**治标**：真正的省法是上面两条"别整份物化"。
- **待做（A2）**：冷缓存下载仍是"MemoryStream → 字典 → 再序列化 17.5MB 落盘"三次物化（只在首次/刷新时走），可改成流式写 `.tmp` + `Move`；因为要动缓存文件形状且只能靠真实冷缓存验证，暂缓。
...
- ⚠️ **构造函数里的落地顺序（2026-09-23 定）**：**主题 → `ExtendsContentIntoTitleBar = true` / `SetTitleBar` → 透明 caption 色（`AppWindow.TitleBar.*BackgroundColor`）→ `ApplyBackdrop()`**。理由：`App.OnLaunched` 里 `new MainWindow()` 紧跟 `Activate()`，构造返回后立刻会画——主题排在 `InitializeAppAsync` 里（原来还排在首屏 `GoToPage` 之后）会让第一帧按系统主题画（系统深色 + 应用浅色 = 顶上闪一下），首屏页面也会在旧主题下被建出来；backdrop 排在 caption 色之前则第一帧顶上先露一条系统 caption 色。⚠️ 构造阶段 `SettingsVM` 还没 `LoadFromConfig()`，主题要直接读 `ConfigService.Config.ThemeMode`（故 `ApplyThemeAndChrome(bool refreshLibrary, string? themeMode)` 带参数，且构造时 `refreshLibrary: false` —— 库还没加载，跑刷新等于提前触发一次扫描）
...
- **两档宿主并存的代价**（网格实现方式）：`ListView` 与 `GridView` 同在一个页面、靠 `Visibility` 二选一；**两档都实体化过之后**会各留一份容器——实测冷启动 128MB → 进页面 156MB（列表）/ 161MB（网格，比列表只贵 6MB，就是 450px 位图的差）→ **两档都切过再 +46MB**（此后每轮切换只再涨 ~1MB，是平台期不是泄漏）。封面位图两档共用且只建一次，所以切档不再有图像表面 churn（也就不需要再补 GC）
...
- 线程与解码：`CoverImageService` 只返回文件路径（不碰 WinUI 类型）；`BitmapImage` 由 VM 在 UI 线程构造，**一律按原生尺寸解**——读文件字节 → `SetSourceAsync`，**不设 `DecodePixelWidth`**（落盘 460×215 正好覆盖网格图框的 450 物理像素，显示端一律缩小，任何 DPI 都清晰）。⚠️ **别用"按视图/按缩放算解码宽度"那类做法**，两条都踩过：① 按视图换宽度（列表 120 / 网格 200）时，切档后任何一次没重建到就会带着另一档的宽度显示（网格里 1.67× 放大 = 糊）；② `DecodePixelType=Logical` 是按**解码那一刻的缩放**取表面的，系统在久置 / 息屏 / 图像表面被回收后重新取一次，就可能取到比显示框小的表面 = 糊，而"重建位图"（刷新按钮）又恢复 —— 症状就是"时糊时清"且极难复现。搜索页缩略图（`SearchViewModel.CreateBitmapAsync`）用的就是这套、从没糊过，两条路径现已统一。⚠️ 探针 `bitmap.PixelWidth` 读到的是原图尺寸、不是上屏表面，别拿它当判据
...
## 15. 内存占用（2026-09-22 优化与实测边界）

- **基线**：WinUI 3 + 自包含 WASDK + 一个 2320×1669 亚克力窗口，**空窗启动就 ~128MB 私有提交 / ~186MB 工作集**（225% DPI）。任务管理器那列是"**活动私有工作集**"，与 .NET 私有提交不是同一口径（同一进程实测差 60MB）→ **看趋势和增量，别拿绝对值互相印证**。
- **入库的增量已经很小**：实测 **+8~19MB**（三档分别 8.3 / 18.6）。剩下的大头是框架的**本地内存**（XAML 表面 / WASDK），**GC 旋钮管不到**。
- **做过的四刀**（细节见 §4 / §3）：

  | 刀 | 实测效果 |
  |---|---|
  | Sudama 整份字典 → DOM → **流式扫描取键** | 单次入库内存增量 **~110MB → +6.1MB**（中间态 DOM 也只有 +41.1MB） |
  | manifest 下载改**流式落盘**（`.part` → `Move`） | 单份清单不再整块进内存 |
  | **删掉 5 秒一次的全库扫描**（三个计数无人读） | 去掉周期 churn：静置 2 分钟**一直 128.0~128.6MB，纹丝不动** |
  | 峰值后 `OstMemory.CompactAfterLargeBuffers()` | 峰值后能回落（**治标**，治本靠前三刀） |

- **实测无效、别再试**：`GCConserveMemory`（用 `DOTNET_GCConserveMemory` 环境变量免重建快测）跑 0 / 5 / 9 三档，**绝对峰值一致**（156.9 / 152.0 / 152.7MB），静置回落也没区别 → **该旋钮对本项目无收益**。**不做进 UI**：那要"写 exe 旁 runtimeconfig + 重启 + 权限失败分支"，代价远大于 0 收益。
- **不建议动的**：`HeapHardLimit*`（逼近上限会 OOM）、`InvariantGlobalization`（可能影响中文文化格式化）、`ConcurrentGarbageCollection=false`（UI 会卡）。
- **结论：内存优化到此为止（2026-09-22 决定）**——剩余占用是框架基线，代码层面压不动。
