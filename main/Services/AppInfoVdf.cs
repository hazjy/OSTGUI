...
/// <summary>
/// 从 <c>&lt;Steam&gt;\appcache\appinfo.vdf</c> 取"客户端已知的 app 与名字"（实测 492 条，读一遍几十毫秒）。
///
/// 两个用途：
/// - 成就页的**正版候选池**：客户端自己认识哪些 app 就以它为准（不再按"有没有成就定义"筛，
///   否则没启动过、也没缓存的拥有游戏根本不出现），再批量做拥有判定；
/// - **名字**：在线补名走 store.steampowered.com，这台机器上被加速器接管、经常不通，
///   而 schema 的 gamename 可能是开发代号（实测 3751950 是 OBSIDIAN）。
///
/// v29 的格式（实测确认）：
...
/// 只做最小解析：找到 "name" 的键 ID，然后每个条目里取第一个 <c>\x01&lt;ID&gt;</c> 后面的字符串（= common.name）。
...
public static class AppInfoVdf
{
    ...
    /// <summary>所有条目：appid → 游戏名（该条目没有名字时给空串）</summary>
    public static Dictionary<string, string> GetAll(string steamPath)
    {
        ...
        try
        {
            ...
            while (offset + 8 <= bytes.Length)
            {
                ...
                result[appId.ToString()] = ReadStringValue(bytes, offset + 8, end, nameKey) ?? "";
                offset += 8 + (int)size;
            }
        }
        catch (Exception ex)
